...
export default function AuctionsPage() {
  ...
  for (let index = 0; index < 16; index++) {
    AUCTIONS_ITEMS.push(<ItemTemplate key={index} id={index} edit={false} />);
  }
  ...
  return (
    <section className="flex-grow background">
      <div className="flex flex-col lg:mx-20 lg:my-4 border-4 border-black justify-center bg-gray-400 gap-4 rounded-lg bg-opacity-25">
        <Header />
        <div className="items_scroll h-60vh flex flex-wrap gap-5 justify-center overflow-y-auto border-r-8 border-transparent ">
          {AUCTIONS_ITEMS}
        </div>
      </div>
    </section>
